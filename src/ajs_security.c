...
static short isSecurityEnabled = FALSE;
static AJ_PermissionRule* securityRules;
...
void AJS_SetSecurityRules(const AJ_PermissionRule* rules, const unsigned int numRules, duk_context* ctx)
{
    if (securityRules) {
...
uint32_t AJS_GetAllJoynSecurityProps(duk_context* ctx, duk_idx_t enumIdx)
{
   ...
   for(int i = 1; duk_next(ctx, -1, 1) ; ++i) {
       ...
       if (strcmp(property, "expiration") == 0) {
           ...
           if (!ecspeke_password) {
               duk_error(ctx, DUK_ERR_REFERENCE_ERROR, "password is required for speke suit");
           }
...
AJ_Status AJS_EnableSecurity(duk_context* ctx)
{
   ...
   status = AJ_BusEnableSecurity(AJS_GetBusAttachment(), suites, 1);
   if (status != AJ_OK) {
